...
import { X, Users, Radio, AlertCircle, Maximize2 } from "lucide-react";
...
// Detect if running on TV or large screen
const isTV = () => {
...
const VideoPlayerImproved = ({ channel, channels, onClose, onChannelChange }: VideoPlayerProps) => {
  ...
  useEffect(() => {
    ...
    // Video.js options with enhanced configuration
    const options = {
      autoplay: true,
      controls: true,
      responsive: true,
    ...
    const player = videojs(videoElement, options, function onPlayerReady() {
      console.log('Video.js player ready for:', channel.name);
      ...
      player.on('error', () => {
        ...
        if (err) {
          switch (err.code) {
            case 1:
              setError("স্ট্রিম লোড বাতিল হয়েছে");
              break;
            case 2:
            ...
            case 3:
              ...
              break;
            case 4:
              setError("স্ট্রিম ফরম্যাট সাপোর্টেড নয়");
              break;
            default:
  ...
  return (
    <div 
      ...
      className="fixed inset-0 z-50 bg-black animate-fade-in flex flex-col"
    >
      {/* Header with Channel Info */}
      {!isFullscreen && (
        <div className="flex items-center justify-between p-3 sm:p-4 bg-black/80 border-b border-white/10 backdrop-blur-md">
          <div className="flex items-center gap-2 sm:gap-3 flex-1">
            ...
            <div className="hidden sm:block flex-1">
              <h2 className="font-display font-semibold text-white text-sm sm:text-base">{channel.name}</h2>
          ...
          <Button 
            ...
            size="icon" 
            onClick={onClose}
            className="text-white hover:bg-white/10 focus:ring-2 focus:ring-white/30 h-10 w-10"
            tabIndex={0}
        ...
        {isLoading && !error && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/60 pointer-events-none backdrop-blur-sm">
            <div className="text-center">
              ...
              <p className="text-white/80 font-medium text-sm">স্ট্রিম লোড হচ্ছে...</p>
            </div>
          </div>
        )}
      ...
      {!isFullscreen && (
        <div className="bg-black/80 border-t border-white/10 backdrop-blur-md p-3 sm:p-4 max-h-32 overflow-y-auto">
          <Carousel
            opts={{
              align: "start",
              loop: true,
              ...
              {otherChannels.map((ch, index) => (
                <CarouselItem key={ch.id} className="pl-2 basis-auto">
                  <button
                    ...
                    type="button"
                    className="flex flex-col items-center gap-1.5 sm:gap-2 cursor-pointer group outline-none focus:scale-110 transition-all duration-200"
                    tabIndex={0}
                  ...
                  >
                    <div className="relative">
                      <img
                        src={ch.logo}
                        alt={ch.name}
                        className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-lg object-cover border-2 border-transparent group-hover:border-blue-400 group-focus:border-blue-400 transition-colors duration-200 shadow-lg"
                      />
                      {ch.isLive && (
...
export default VideoPlayerImproved;
